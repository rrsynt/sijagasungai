...
function EducationContent() {
  ...
  // Callback from QuizPanel when finished to award badge
  const handleQuizFinish = (earnedBadge: string) => {
    setBadges(prev => {
    ...
    <div className="min-h-screen bg-[#fafaf8] py-16 sm:py-24 relative">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16 border-b border-gray-200 pb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          {/* Persistent badges collection */}
          {badges.length > 0 && (
            <div className="bg-white border-2 border-gray-900 p-4 w-full md:w-auto shadow-sm">
              <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-2">{t('Koleksi Badge Anda', 'Your Badges')}</span>
              <div className="flex flex-wrap gap-2">
                {badges.map(b => (
                  <span key={b} className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 border border-amber-300 text-xs font-bold text-amber-900 rounded-full" title={b}>
                    {b}
                  </span>
                ))}
              </div>
